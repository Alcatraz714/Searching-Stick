...
namespace Gameplay {
	namespace Collection {
		...
		void StickCollectionContoller::sortElements()
		{
			...
			std::sort(sticks.begin(), sticks.end(), [this](const Stick* a, const Stick* b) { return compareElementsByData(a, b); });// Comparsion to get the appropriate half to search
		...
		void Gameplay::Collection::StickCollectionContoller::searchElement(SearchType search_type)
		{
			...
			case Gameplay::Collection::SearchType::BINARY_SEARCH:
				sortElements(); // binary search only works on sorted groups
				time_complexity = "O(Log n)"; // tentative for now
